...
const mocks = vi.hoisted(() => ({ embedQuery: vi.fn(), vectorSearch: vi.fn(), generateAnswer: vi.fn() }));
...
vi.mock("@/lib/rag/generate-answer", () => ({ generateAnswer: mocks.generateAnswer }));

import { POST } from "./route";

const request = (body: unknown) => new Request("http://localhost/api/search", { method: "POST", body: typeof body === "string" ? body : JSON.stringify(body), headers: { "content-type": "application/json" } });
...
describe("POST /api/search", () => {
	...
	it.each([undefined, null, "", "   ", 42])("rejects invalid query %j", async (query) => expect((await POST(request({ query }))).status).toBe(400));
	...
	it("defaults topK and returns answer and sources", async () => {
		...
		expect(await response.json()).toEqual({ answer: "formatted answer", sources: results });
		expect(mocks.embedQuery).toHaveBeenCalledWith("q");
		expect(mocks.vectorSearch).toHaveBeenCalledWith([1, 2], 8);
		expect(mocks.generateAnswer).toHaveBeenCalledWith({ userQuery: "q", chunks: results });
	});
	it("passes explicit topK and supports no results", async () => {
		...
		expect(await response.json()).toEqual({ answer: "no results", sources: [] });
		expect(mocks.vectorSearch).toHaveBeenCalledWith([1, 2], 3);
	});
	it.each(["embedQuery", "vectorSearch", "generateAnswer"])("returns 500 when %s fails", async (dependency) => {
		mocks[dependency as keyof typeof mocks].mockRejectedValueOnce(new Error("dependency down"));
		const response = await POST(request({ query: "q" }));
		expect(response.status).toBe(500);
		expect(await response.json()).toMatchObject({ error: "Search failed", message: "dependency down" });
	});
});
