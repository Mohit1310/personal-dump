...
describe("DumpForm", () => {
	...
	it("posts the selected type and content", async () => {
		...
		await user.type(
			screen.getByPlaceholderText(/input stream ready/i),
			"Error details",
		);
		...
		await waitFor(() =>
			expect(fetchMock).toHaveBeenCalledWith(
				...
				expect.objectContaining({
					body: JSON.stringify({ content: "Error details", type: "error" }),
				}),
			),
		);
	});
	...
	it("clears after success and retains content after failure", async () => {
		...
		await waitFor(() =>
			expect(toast.error).toHaveBeenCalledWith("Failed to store knowledge"),
		);
